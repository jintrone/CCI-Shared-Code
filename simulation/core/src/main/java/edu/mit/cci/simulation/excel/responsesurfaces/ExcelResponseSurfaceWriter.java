...
public class ExcelResponseSurfaceWriter {
    ...
    public static <T extends Comparable<T>, U extends Comparable<U>> void writeSpreadSheet(String filename, int numInputsAndOutputs, SimpleResponseSurface<T, U> responseSurface) {
        ...
        int beginYear = 2000; //Beginning Year of Input
        int dY = 10; //Year interval between Input Years
        ...
        HSSFRow dataSec1 = dataSheet.createRow(row);
        row++;
        dataSec1.createCell(0).setCellValue("SLICE EQUATIONS");
        HSSFRow dataHeader1 = dataSheet.createRow(row);
        row++;
        ...
        for (int i = 0; i < ndx.size(); i++) {
            HSSFRow curRow = dataSheet.createRow(row);
            row++;
            ...
            for (SliceSegment<T, U> seg : responseSurface.getAtIndex(ndx.get(i))) {
                curRow.createCell(col).setCellValue(seg.function.getParam(1));
                col++;
                curRow.createCell(col).setCellValue(seg.function.getIntercept());
                col++;
            }
        ...
        dataSheet.createRow(row).createCell(0).setCellValue("SLICE BOUNDARIES");
        row++;//Section TWO starts
        HSSFRow dataHeader2 = dataSheet.createRow(row);
        row++;
        ...
        for (int i = 0; i < ndx.size(); i++) {
            ...
            T top = null;
            for (SliceSegment<T, U> seg : responseSurface.getAtIndex(ndx.get(i))) {
                curRow.createCell(col).setCellValue(new Double(seg.fromCriterion.toString()));
                col++;
                top = seg.toCriterion;
            }
            curRow.createCell(col).setCellValue(new Double(top.toString()));
            col++;
        }
        ...
        for (int i = 0; i < numInputsAndOutputs; i++) {
            ...
            HSSFRow curRow = ioSheet.createRow(row);
            row++;
    ...
    public static void main(String[] args) {
        ...
        SliceSegment a = new SliceSegment(0.122, 0.135, 2020, new Polynomial(a1));
        SliceSegment b = new SliceSegment(0.081, 0.108, 2030, new Polynomial(a1));
        SliceSegment c = new SliceSegment(0.075, 0.995, 2040, new Polynomial(a1));
        Slice s = new Slice();
        s.add(a);
        s.add(b);
        s.add(c);
        ...
        ExcelResponseSurfaceWriter.writeSpreadSheet("testing.xls", 11, srs);
